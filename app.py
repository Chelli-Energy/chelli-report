...
def send_pdf_via_email(pdf_bytes: bytes, filename: str, to_email: str):
    # Usa SMTP da st.secrets["EMAIL"]
    s = st.secrets["EMAIL"]  # attesi: SMTP_SERVER, SMTP_PORT, SMTP_USER, SMTP_PASS
    msg = EmailMessage()
    msg["From"] = formataddr(("Chelli Report", s["SMTP_USER"]))
    # invio anche a assistenza come nella tua versione
    msg["To"] = f"{to_email}, assistenza@chellienergysolutions.it" if to_email else "assistenza@chellienergysolutions.it"
    msg["Cc"] = ""
    ...
    # Corpo HTML con logo CID inline
    html_body = """
    <html>
      <body style="font-family:Arial,sans-serif; color:#222;">
        <p>Gentile cliente,<br>
        trasmettiamo in allegato il report mensile del suo impianto fotovoltaico contenente i dati registrati.
        Può contattare il nostro servizio tecnico per approfondimenti e informazioni.</p>

        <hr style="border:0; border-top:1px solid #ccc; margin:20px 0;" />
        <img src="cid:logo" alt="Logo Chelli Energy Solution"
             width="140" style="display:block; margin-bottom:10px;">
        ...
        <p style="font-size:8px; color:#666; margin-top:18px; line-height:1.3;">
          ...
          compresi eventuali allegati, sono confidenziali e riservate ai sensi del D.Lgs. 196/2003 e del GDPR 679/2016.
          Se avete ricevuto per errore questo messaggio, contattate il mittente e cancellate la e-mail.
        </p>
      </body>
    </html>
    """
    ...
    # Allego il logo come inline related per il part HTML
    try:
...
def main():
    ...
    try:
        ...
        df["Rete_immessa_kWh"] = pd.to_numeric(df["Energia alimentata nella rete"], errors="coerce").fillna(0.0) / 1000.0
        df["Rete_prelevata_kWh"] = pd.to_numeric(df["Energia prelevata dalla rete"], errors="coerce").fillna(0.0) / 1000.0
        ...
        if st.button("Spedisci PDF", key="send_pdf"):
            try:
                cliente_email = row.get("email", "") if selected else ""
